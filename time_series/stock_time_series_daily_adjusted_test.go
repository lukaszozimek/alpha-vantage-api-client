...
func TestGetDailyAdjusted(t *testing.T) {
	...
	c.HttpClient.Transport = roundTripFunc(func(r *http.Request) (*http.Response, error) {
		assert.Equal(t, "https://www.alphavantage.co/query?function=TIME_SERIES_DAILY_ADJUSTED&symbol=EUR&apikey=deomo", r.URL.String())
		return &http.Response{
			StatusCode: http.StatusOK,
			Body:       ioutil.NopCloser(strings.NewReader(`{}`)),
		}, nil
	})
	item := time_series.GetDailyAdjusted("EUR", "deomo", c)
